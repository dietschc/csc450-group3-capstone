...
function AuthAccount(props) {
    ...
    // Saving the param Id
    return null
    // return (users.length > 0 && (users[0].isLoggedIn && users[0].userId ===  ? children : <Navigate to="/login" replace state={{ path: location.pathname }}/>);
  }
...
export default connect(mapStateToProps, null)(AuthAccount);