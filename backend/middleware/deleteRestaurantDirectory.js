...
const deleteRestaurantDirectory = async (directory) => {
    ...
    try {
        ...
        try {
            ...
            console.log("Restaurants directory deleted Successfully")
            return 1;
        }
        // Something went wrong trying to delete the directory
        catch (err) {
            console.log("Error deleting restaurants directory : " + JSON.stringify(err));
            return -2;
        }
        // Otherwise the directory was not found
    } catch (err) {
        console.log("Restaurants directory not Found ERROR : " + err.code);
        return -1;
    }
};
